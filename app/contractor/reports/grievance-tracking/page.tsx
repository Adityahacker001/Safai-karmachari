...
import { MessageSquare, Download, Clock, ThumbsUp, Eye, Search } from "lucide-react";
    ...
    <div className="min-h-screen p-6 md:p-12 bg-gradient-to-br from-blue-100 via-purple-100 via-pink-100 via-rose-100 to-yellow-100 space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <Card className="border-0 rounded-2xl bg-gradient-to-br from-green-500 to-emerald-600 text-white shadow-xl transform transition-all duration-300 hover:scale-105">
                    <CardHeader className="pb-2 flex flex-row items-center justify-between">
                        <CardTitle className="text-base font-medium text-green-100">Top Grievance Category</CardTitle>
                        <ThumbsUp className="h-6 w-6 text-green-200" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-4xl font-bold">{topCategory}</div>
                        <p className="text-sm text-green-100 mt-1">Most common complaint type</p>
                    </CardContent>
                </Card>
                ...
                <CardHeader className="p-8 md:p-10 bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-700 text-white shadow-lg">
                    ...
                    <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4 pt-6">
                        <div className="relative flex-grow">
                            ...
                            <Input placeholder="Search by worker, ID, or category..." className="pl-12 pr-4 py-2.5 max-w-full sm:max-w-xs bg-white border-gray-300 focus:border-red-500 focus:ring-2 focus:ring-red-200 rounded-xl placeholder:text-black" />
                        </div>
                        <Select value={statusFilter} onValueChange={setStatusFilter}>
                            <SelectTrigger className="w-full sm:w-[180px] bg-white border-gray-300 focus:border-red-500 focus:ring-2 focus:ring-red-200 rounded-xl px-4 py-2.5 placeholder:text-black">
                                <span className={statusFilter === "all" ? "text-black font-bold" : "text-black"}>
                                    {statusFilter === "all" ? "All Statuses" :
                                        statusFilter === "pending" ? "Pending" :
                                        statusFilter === "resolved" ? "Resolved" :
                                        statusFilter === "escalated" ? "Escalated" : statusFilter}
                                </span>
                            </SelectTrigger>
                            <SelectContent className="shadow-xl rounded-lg border-gray-200">
                                <SelectItem value="all" className="text-black font-bold">All Statuses</SelectItem>
                    ...
                    <Table className="w-full">
                        <TableHeader className="bg-gray-50 rounded-t-xl">
                            <TableRow className="border-b border-gray-200">
                                <TableHead className="py-4 px-6 text-gray-700 font-bold text-base">Case ID</TableHead>
                                <TableHead className="py-4 px-6 text-gray-700 font-bold text-base">Worker</TableHead>
                                <TableHead className="py-4 px-6 text-gray-700 font-bold text-base">Category</TableHead>
                                <TableHead className="py-4 px-6 text-gray-700 font-bold text-base">Submitted</TableHead>
                                <TableHead className="py-4 px-6 text-gray-700 font-bold text-base">Resolved</TableHead>
