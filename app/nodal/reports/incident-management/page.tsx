      ...
      <div className="py-8 px-4 sm:px-8 md:px-12 lg:px-24 xl:px-32 space-y-8 w-full">
        ...
        <Card className="border border-gray-300 rounded-lg shadow-sm">
          ...
          <CardContent className="p-0">
            <div className="p-4 border-b bg-gradient-to-r from-white via-slate-50 to-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 shadow-sm">
              <div className="flex items-center gap-3">
                <input placeholder="Search by Case ID, contractor or type" className="px-3 py-2 border border-gray-200 rounded-md text-sm w-64 focus:ring-1 focus:ring-blue-500" />
                <select className="px-3 py-2 border border-gray-200 rounded-md text-sm bg-white">
                  <option>All Status</option>
                  <option>Action Required</option>
                  <option>Action Overdue</option>
                  <option>Resolved</option>
                  <option>Escalated</option>
                </select>
              </div>
              <div className="text-sm text-slate-500">Showing {incidents.length} records</div>
            </div>

            <div className="p-4 overflow-x-auto">
              <Table className="min-w-full text-sm">
                <TableHeader>
                  <TableRow className="bg-gradient-to-r from-white to-slate-50 border-b border-slate-200 sticky top-0 z-10 backdrop-blur-sm">
                    <TableHead className="px-6 py-3 text-xs font-semibold text-slate-700 uppercase tracking-wide text-left">
                      Case ID
                    </TableHead>
                    <TableHead className="px-6 py-3 text-xs font-semibold text-slate-700 uppercase tracking-wide text-left">
                      Type
                    ...
                    <TableHead className="px-6 py-3 text-xs font-semibold text-gray-700 uppercase tracking-wide text-right">
                      Actions
                    </TableHead>
                  </TableRow>
                </TableHeader>
                  ...
                  {incidents.map((incident, i) => (
                    <TableRow
                      ...
                      className={cn(
                        ...
                        "hover:bg-slate-100 transition-shadow hover:shadow-sm"
                      )}
                    >
                      <TableCell className="px-6 py-4 font-medium text-blue-600">{incident.id}</TableCell>
                      <TableCell className="px-6 py-4 text-gray-700">{incident.type}</TableCell>
                      <TableCell className="px-6 py-4 text-gray-700">{incident.contractor}</TableCell>
                      <TableCell className="px-6 py-4 text-gray-700">{incident.reported}</TableCell>
                        ...
                        <Dialog onOpenChange={(isOpen) => !isOpen && setSelectedIncident(null)}>
                          <DialogTrigger asChild>
                            <Button
                              variant="outline"
                              size="sm"
                              className="inline-flex items-center gap-2 px-3 py-1.5 rounded-md border border-slate-200 text-slate-700 bg-white hover:bg-slate-50"
