      ...
      <div className="max-w-7xl mx-auto">
         ...
         <header className="mb-6">
           ...
           <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
             <div className="flex items-center space-x-3">
               <span className="p-2 bg-gradient-to-br from-indigo-500 to-blue-500 rounded-full text-white shadow-lg"><CalendarCheck className="w-8 h-8" /></span>
               <div> <h1 className="text-3xl font-bold text-slate-800">Annual Report Summary</h1> <p className="text-slate-500 mt-1">Yearly summary of scheme performance and fund utilization.</p> </div>
             </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 border border-slate-100 mb-6">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-4">
             ...
             <FormSelect label="District" name="district" value={filters.district} onChange={handleFilterChange}><option value="">All Districts</option>{mockDistricts.filter(d=> filters.state ? MOCK_ANNUAL_DATA.find(data => data.state === filters.state) : true).map(d=><option key={d} value={d}>{d}</option>)}</FormSelect> {/* Simplified district filtering */}
             ...
             <table className="w-full text-sm">
               <thead className="bg-indigo-100 text-indigo-800 font-semibold sticky top-0 z-10">
                 <tr>
                   ...
                   {/* Add other beneficiary columns if needed in summary view */}
               ...
               <tbody className="divide-y divide-slate-200">
                 ...
                 {paginatedData.length === 0 && (<tr><td colSpan={14} className="text-center py-10 text-slate-500">No records found.</td></tr>)}
               </tbody>
             </table>
           </div>
           {/* Pagination */}
