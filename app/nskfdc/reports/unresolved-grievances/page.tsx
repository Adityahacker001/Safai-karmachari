      ...
      <div className="max-w-7xl mx-auto">
        {/* Header */}
         <header className="mb-6">
           <nav className="flex items-center text-sm font-medium text-slate-500 mb-2"> <LayoutDashboard className="w-4 h-4 mr-1.5" /> Dashboard <ChevronRightIcon className="w-4 h-4 mx-1" /> Reports & Analytics <ChevronRightIcon className="w-4 h-4 mx-1" /> <span className="font-semibold text-indigo-600">Unresolved Grievance Report</span> </nav>
           <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
             <div className="flex items-center space-x-3">
               <span className="p-2 bg-gradient-to-br from-red-500 to-orange-500 rounded-full text-white shadow-lg"><MessageSquareWarning className="w-8 h-8" /></span>
               <div> <h1 className="text-3xl font-bold text-slate-800">Unresolved Grievance Report</h1> <p className="text-slate-500 mt-1">Monitor pending grievances, types, durations, and authorities.</p> </div>
             </div>
             ...
             <table className="w-full text-sm">
               <thead className="bg-indigo-100 text-indigo-800 font-semibold sticky top-0 z-10">
                 <tr>
                   ...
                   <SortableHeader colKey="benName" title="Beneficiary Name" sortConfig={sortConfig} requestSort={requestSort} className="min-w-[150px]" />
                   <th className="px-4 py-3 text-left min-w-[100px]">Type</th>
                   <th className="px-4 py-3 text-left min-w-[180px]">Scheme</th>
               ...
               <tbody className="divide-y divide-slate-200">
                 {paginatedData.map((item, index) => (
                   <tr key={item.id} className="hover:bg-indigo-50/70 transition-colors">
                     ...
                     <td className="px-4 py-3 text-slate-700 font-medium">{item.grievanceId}</td>
