...
import {
  Users, FileText, BarChart3, Shield, Building, MapPin, Globe,
  UserPlus, Clock, AlertTriangle, GraduationCap, DollarSign,
  CheckSquare, Award, ChevronDown, ChevronRight, LogOut,
} from "lucide-react";
...
const roleConfig = {
    ...
    district: {
        ...
        dataEntry: [ { name: 'Recognition Review', icon: Award, href: '/district/data-entry/recognition-review' }, ],
        reports: [ { name: 'District Compliance', icon: CheckSquare, href: '/district/reports/district-compliance' }, { name: 'Recognition System', icon: Award, href: '/district/reports/recognition-system' }, { name: 'Unit Performance', icon: Building, href: '/district/reports/unit-performance' }, { name: 'All Reports', icon: FileText, href: '/district/reports/All-reports' }, ],
    },
    state: {
        title: 'State Command Centre', icon: MapPin, color: 'text-purple-400', bgColor: 'from-purple-900/30 to-slate-900',
        dataEntry: [ { name: 'State-wide Compliance', icon: CheckSquare, href: '/state/data-entry/statewide-compliance' }, { name: 'Fund Allocation', icon: DollarSign, href: '/state/data-entry/fund-allocation' }, { name: 'Policy Implementation', icon: FileText, href: '/state/data-entry/policy-implementation' }, ],
        reports: [ { name: 'State Compliance', icon: BarChart3, href: '/state/reports/state-compliance' }, { name: 'Fund Allocation Reports', icon: DollarSign, href: '/state/reports/fund-allocation' }, { name: 'Policy Tracking', icon: FileText, href: '/state/reports/policy-tracking' }, { name: 'All Reports', icon: FileText, href: '/state/reports/All-reports' }, { name: 'District Performance', icon: Building, href: '/state/reports/district-performance' }, ],
    },
    national: {
        title: 'National Dashboard', icon: Globe, color: 'text-indigo-400', bgColor: 'from-indigo-900/30 to-slate-900',
        ...
        reports: [ { name: 'National Overview', icon: Globe, href: '/national/reports/national-overview' }, { name: 'Directive Tracking', icon: FileText, href: '/national/reports/directive-tracking' }, { name: 'Grievance', icon: Award, href: '/national/reports/grievance' }, { name: 'Reports', icon: FileText, href: '/national/reports/reports' }, { name: 'Performance Report', icon: FileText, href: '/national/reports/Performance-reports' }, { name: 'Recognition Leaderboard', icon: Award, href: '/national/reports/recognition-leaderboard' }, { name: 'Audit Logs', icon: Award, href: '/national/reports/Audit-logs' }, { name: 'User Management', icon: Award, href: '/national/reports/User-Management' }, { name: 'Initiate Award', icon: Award, href: '/national/reports/initiate-award' }, ],
    },
};
...
export default function Sidebar({ role = "national" }: SidebarProps) {
  ...
  const NavLink = ({ href, icon: Icon, children, activeClass, inactiveClass, isDashboard = false }: NavLinkProps) => {
    const isActive = pathname === href;
    const baseClass = "flex items-center space-x-3 rounded-lg text-sm font-medium relative group transition-all duration-300";
  ...
  <div className="fixed top-0 left-0 flex flex-col w-64 h-screen bg-gradient-to-b from-blue-900 to-indigo-950 text-white shadow-2xl border-r border-blue-800 z-50">
      ...
      {/* Header */}
  <div className={cn("relative p-4 flex items-center space-x-3 bg-gradient-to-r shadow-lg border-b border-white/10 z-10", config.bgColor)}>
      ...
      <div className="flex-1 overflow-y-auto relative z-10">
        <nav className="p-4 space-y-4">
          ...
          <div className="space-y-2">
            <button onClick={() => setIsDataEntryOpen(!isDataEntryOpen)} className="w-full flex items-center justify-between px-4 py-2 text-sm font-bold text-gray-200 bg-white/5 rounded-lg hover:bg-white/10 transition-all">
          ...
          {/* Reports Section */}
          <div className="space-y-2">
            <button onClick={() => setIsReportsOpen(!isReportsOpen)} className="w-full flex items-center justify-between px-4 py-2 text-sm font-bold text-gray-200 bg-white/5 rounded-lg hover:bg-white/10 transition-all">
      ...
      <div className="p-4 border-t border-white/10 bg-white/5 relative z-10">
        <a href="/" className="flex items-center space-x-3 px-4 py-2 rounded-xl text-sm font-medium text-red-400 hover:text-red-200 hover:bg-red-500/10 transition-all relative group">
